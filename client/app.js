...
async function initializeDiary(secretPhrase) {
    ...
    try {
        ...
        if (data.initialize.success) {
            ...
            showStatus('Diary created successfully! 🎉', 'success');
            return true;
        } else {
...
async function addEntry(title, content, secretPhrase) {
    ...
    try {
        setLoading(true);
        const data = await client.mutate(mutation, { title, content, secretPhrase });
    ...
    grid.innerHTML = state.entries
        ...
        .map(entry => `
            <div class="entry-card fade-in" data-entry-id="${entry.id}">
                ...
                <p class="entry-preview">${escapeHtml(truncateText(entry.content))}</p>
                <div class="entry-actions">
                    <button class="btn btn-sm btn-outline" onclick="viewEntry('${entry.id}')">
                        📖 Read
                    </button>
                    <button class="btn btn-sm btn-ghost" onclick="editEntry('${entry.id}')">
                        ✏️ Edit
                    </button>
                    <button class="btn btn-sm btn-ghost text-error" onclick="confirmDeleteEntry('${entry.id}')">
                        🗑️ Delete
                    </button>
                </div>
            </div>
...
function viewEntry(entryId) {
    const entry = state.entries.find(e => e.id.toString() === entryId.toString());
    if (!entry) return;

    document.getElementById('viewEntryTitle').textContent = entry.title;
    document.getElementById('viewEntryDate').textContent = formatDate(entry.timestamp);
...
function setupEventListeners() {
    ...
    if (deleteEntryForm) deleteEntryForm.addEventListener('submit', handleDeleteEntry);
